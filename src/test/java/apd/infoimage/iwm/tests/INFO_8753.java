 ...
 * INFO-8753 
 * This class Performs 'Get Next' functionality for multiple workitems from any queue by sending the workitems to default queue  
 * Pre-requisite : Data Entry should be empty. All the workitems in Data Entry closing is possible but throws Exception.
 */
public class INFO_8753 extends SuperClassIWM {
	@BeforeMethod
	public void setUp() {
	...
	public void testGetNextMultipleWorkitemsInQueueModifyingWorkitemNavigatingToWorkitems() {
		Log.startTestCase("INFO_8753_GetNextAutoViewWorkitemQueueSendingWorkitemsDefaultQueueTest");
		try {
			...
			for(arrCounter = 0; arrCounter < wiCountVal; arrCounter++)
			{
				if(arrCounter < wiCountVal)
				{
					...
					if(workitemPresence){
						...
						ATUReports.add("Performs 'Get Next' functionality for multiple workitems from any queue by sending the workitems to default queue", "","Workitem should be displayed",
			...
			Log.endTestCase("INFO_8753_GetNextAutoViewWorkitemQueueSendingWorkitemsDefaultQueueTest");
