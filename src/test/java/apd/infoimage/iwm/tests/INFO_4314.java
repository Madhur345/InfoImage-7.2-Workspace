 ...
 * @author SumanGaK
 * INFO-4314
 * This class is used to verify the Nested search functionality with Multi ANDs OR using query builder in iwm. 
 */
public class INFO_4314 extends SuperClassIWM{
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiANDsOR() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-4314 is Test to verify the Nested search functionality with Multi ANDs OR using query builder in iwm.");
		ATUReports.setAuthorInfo("Suman", "JUN-2018", "0.3");

		try {
			Log.startTestCase("INFO_4314_VerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiANDsORTest");
			...
			String nestedQuery=ExcelLib.getCellValue(xlpath, sheet, 69, 1);
			...
			sp.getIdCodeFieldInORNestedSearch().clear();
			sp.getIdCodeFieldInORNestedSearch().sendKeys(idCodeStr);
			Reporter.log("IdCodeField In OR value is entered", true);
			Log.info("IdCodeField In OR value is entered");
			ATUReports.add("IdCodeField In OR value is entered", true);

			sp.getMultiANDORInvoiceNoFieldInNestedSearch().clear();
			sp.getMultiANDORInvoiceNoFieldInNestedSearch().sendKeys(invoiceNoStr);
			Reporter.log("invoiceNoStr value is entered", true);
			Log.info("invoiceNoStr value is entered");
			ATUReports.add("invoiceNoStr value is entered", true);
			util.wait(time);
			util.wait(time);
			util.waitForPageToLoad();
			...
			if(workitemPresence){
				Reporter.log("Workitem is searched successfully by Nested query using multiANDsOR",true);
				ATUReports.add("Workitem is searched successfully by Nested query using multiANDsOR", true);
				Log.info("Workitem is searched successfully by Nested query using multiANDsOR");
			}else{
				ATUReports.add("Nested query using multiANDsOR test failed",LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query using multiANDsOR test failed ");
			}
			...
			ATUReports.add("Failed to verify the Nested search functionality with multiANDsOR using query builder in iwm.",LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality with multiANDsOR using query builder test failed");
		}
		finally {
			Log.endTestCase("INFO_4314_VerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiANDsORTest");
