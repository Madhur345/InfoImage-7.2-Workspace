 ...
 * @author PradhanJ
 * INFO-5779
 * To test the nested Query in IWM Using document type of workitems in Domain level 
 * by passing only "ATTRIBUTES"
 * column names in SQL query string that is passed as an input.
 */
public class INFO_5779 extends SuperClassIWM{
	...
	public void testNestedSearchBypassingATTRIBUTENameInSQLQuery() {
		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-5779 is Test to verify the Nested search functionality "
				+ "by passing only ATTRIBUTES column names in SQL query string that is passed as an input.");
		ATUReports.setAuthorInfo("Jayashri", "JUNE-2018", "0.3");

		try {
			Log.startTestCase(
					"INFIO_5779_NestedSearchBypassingATTRIBUTENameInSQLQuery");
			...
			if(f9){
				Reporter.log("Workitem is searched successfully by by passing only ATTRIBUTES "
						+ "column names in SQL query string that is passed as an input.",true);
				ATUReports.add("Workitem is searched successfully by by passing only ATTRIBUTES "
						+ "column names in SQL query string that is passed as an input.", true);
				Log.info("Workitem is searched successfully by by passing only ATTRIBUTES column names in "
						+ "SQL query string that is passed as an input.");
			}else{
				ATUReports.add("Nested query by passing only ATTRIBUTES column names in SQL query string test failed", 
						LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query by passing only ATTRIBUTES column names in SQL query string test failed ");
			}
			...
			ATUReports.add(
					...
					+ "by passing only ATTRIBUTES column names in SQL query string.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality by passing only ATTRIBUTES column names in SQL query string test failed");
		}
		finally {
			Log.endTestCase("INFIO_5779_NestedSearchBypassingATTRIBUTENameInSQLQuery");
