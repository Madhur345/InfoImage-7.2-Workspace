 ...
 * @author PradhanJ
 * INFO-4297
 * Test to verify the Nested search functionality with (>=)greaterthan equals to using query builder in iwm.
 */
public class INFO_4297 extends SuperClassIWM{
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchWithGreaterThanOperator() {
		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-4298 is Test to verify the Nested search functionality "
				+ "(>=)greaterthan equals to using query builder in iwm");
		ATUReports.setAuthorInfo("Jayashri", "MAY-2018", "0.3");

		try {
			Log.startTestCase(
					"INFO_4297_VerifyNestedSearchWithGreaterThanOperator");
			...
			ATUReports.add(
					...
					+ "with >=(Greater Than Equal To) using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality with >=(Greater Than Equal To) test failed");
		}
		finally {
			Log.endTestCase("INFO_4297_VerifyNestedSearchWithGreaterThanOperator");
