...
/**
 * @author PradhanJ INFO-4296 Test to verify the Nested search functionality
 *         with (<=)lessthan equals to using query builder in iwm.
 */
public class INFO_4296 extends SuperClassIWM {
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchWith_LessThanEqualTo_Operator() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-4298 is Test to verify the Nested search functionality "
				+ "with (<=)lessthan equals to using query builder in iwm");
		ATUReports.setAuthorInfo("Jayashri", "MAY-2018", "0.3");

		try {
			Log.startTestCase("INFO_4296_VerifyNestedSearchWith_LessThanEqualTo_Operator");
			...
			util.wait(time);
			// boolean f9 =
			// util.verifyObjectPresentReturnsBool(sp.getWorkItemName(workitem));

			Reporter.log("Workitem is searched successfully by Nested query using <=(Less Than equal To) operator",
					true);
			ATUReports.add("Workitem is searched successfully by Nested query using <=(Less Than equal To) operator",
					true);
			...
			ATUReports.add(
					"Failed to verify the Nested search functionality "
							+ "with <=(Less Than equal To) using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality with <=(Less Than equal To) test failed");
		} finally {
			Log.endTestCase("INFO_4296_VerifyNestedSearchWith_LessThanEqualTo_Operator");
