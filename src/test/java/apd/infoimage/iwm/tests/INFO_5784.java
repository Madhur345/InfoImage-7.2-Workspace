 ...
 * @author PradhanJ
 * INFO-5784
 * To test the Nested Query in IWM for workitems in Domain level by passing "ATTRIBUTES" and "OBJECT" column names 
 * by joining two tables in SQL query string that is passed as an input
 */
public class INFO_5784 extends SuperClassIWM{
	...
	public void testNestedsearchByPassingATTRIBUTEsAndOBJECTColumnInQueryString() {
		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-5780 is Test to verify the Nested search functionality "
				+ "in Domain level by passing ATTRIBUTES and OBJECT column names by joining "
 + "two tables in SQL query string that is passed as an input");
		ATUReports.setAuthorInfo("Jayashri", "JUNE-2018", "0.3");

		try {
			Log.startTestCase(
					"INFO_5784_NestedQueryInDomainLevelByPassingATTRIBUTESandOBJECTcolumns");
			...
			if(f9){
				Reporter.log("Workitem is searched successfully In Domain level by passing ATTRIBUTES and OBJECT column names "
						+ "by joining two tables in query string",true);
				ATUReports.add("Workitem is searched successfully In Domain level by passing ATTRIBUTES and OBJECT column names "
						+ "by joining two tables in query string", true);
				Log.info("Workitem is searched successfully  In Domain level by passing ATTRIBUTES and OBJECT column names by "
						+ "joining two tables in query string");
			}else{
				ATUReports.add("Nested query In Domain level by passing ATTRIBUTES and OBJECT column "
						+ "names by joining two tables in query string test failed", 
						LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query using ATTRIBUTES and OBJECT column test failed ");
			}
			...
			ATUReports.add(
					...
					+ "using ATTRIBUTES and OBJECT column using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality In Domain level using ATTRIBUTES and OBJECT column test failed");
		}
		finally {
			Log.endTestCase("INFO_5784_NestedQueryInDomainLevelByPassingATTRIBUTESandOBJECTcolumns");
