 ...
 * @author PradhanJ
 * INFO-4294
 * Test to verify the Nested search functionality with (>)greaterthan using query builder in iwm.
 */
public class INFO_4294 extends SuperClassIWM{
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchWith_GreaterThan_Operator() {
		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-4294 is Test to Test to verify the Nested search functionality "
				+ "with (>)greaterthan using query builder in iwm");
		ATUReports.setAuthorInfo("Jayashri", "MAY-2018", "0.3");

		try {
			Log.startTestCase(
					"INFO_4294_VerifyNestedSearchWith_GreaterThan_Operator");
			...
			select.selectByVisibleText(operatorGreaterThan);
			...
			ATUReports.add(
					...
					+ "with >(Greater Than) operator using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality with  >(Greater Than) operator test failed");
		}
		finally {
			Log.endTestCase("INFO_4294_VerifyNestedSearchWith_GreaterThan_Operator");
		}
	}
}
