 ...
 * "ATTRIBUTES" and "QUEUES" column names by joining two tables in SQL query string that is passed as an input
 */
public class INFO_5785 extends SuperClassIWM{
	...
	public void testNestedQueryInDomainLevelByPassingATTRIBUTESandQUEUESinQueryString() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-5785 to test the Nested Query in IWM for workitems in Domain level by passing ATTRIBUTEs and QUEUES"
		...
		try {
			Log.startTestCase(
					"INFO_5785_NestedQueryInDomainLevelByPassingATTRIBUTESandQUEUESinQueryString");
			...
			if(f9){
				Reporter.log("Workitem is searched successfully by Nested query "
						+ "in Domain level by passing ATTRIBUTEs and QUEUES",true);
				ATUReports.add("Workitem is searched successfully by Nested query"
						+ " in Domain level by passing ATTRIBUTEs and QUEUES", true);
				Log.info("Workitem is searched successfully by Nested query using AND operator");
			}else{
				ATUReports.add("Nested query in Domain level by passing ATTRIBUTEs and QUEUES test failed", 
						LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query in Domain level by passing ATTRIBUTEs and QUEUES test failed ");
			}
			...
			ATUReports.add(
					...
					+ "in Domain level by passing ATTRIBUTEs and QUEUES",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality in Domain level by passing ATTRIBUTEs and QUEUES test failed");
		}
		finally {
			Log.endTestCase("INFO_5785_NestedQueryInDomainLevelByPassingATTRIBUTESandQUEUESinQueryString");
