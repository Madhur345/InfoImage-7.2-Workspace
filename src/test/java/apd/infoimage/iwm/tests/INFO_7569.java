...
/**
 * @author Biswajit - 06-June-2018 INFO_7569 This class Perform To Export JPG
 *         Image To TIF Format From Document Type Of Workitem In IWM
 */
public class INFO_7569 extends SuperClassIWM {
	...
	public void testSameWorkItemInUserQueue() {
		...
		Log.startTestCase(
				"INFO-7569 This Scenario will export JPG image to TIF format from document type of workitem in IWM");
		...
		try {
			...
			e.printStackTrace();
			Reporter.log("Exporting JPG image to TIF format from document type of workitem is failed.", true);
			ATUReports.add("Exporting JPG image to TIF format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting JPG image to TIF format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO-7569 export JPG image to TIF format from document type of workitem in IWM is Failed");
		}
	}
