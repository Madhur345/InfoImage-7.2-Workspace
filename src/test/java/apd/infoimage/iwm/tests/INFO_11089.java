...
/**
 * @author GuptaPr2
 *         INFO_11089_Test_to_verify_Renaming_the_Document_type_of_workitem_while_passing_supported_character_in_the_Workitem_Name_field
 */
public class INFO_11089 extends SuperClassIWM {
	...
	public void VerifyRenamingDocumentType() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-11089 is to verify Renaming the document type of workitem while passing supported character in the Workitem Name field");
		...
		try {
			...
			String newWitemName = "New&8" + util.getSysDate(0, "yyDDMMhhmmss");

			// getCheckBoxWorkItemName
			...
			Reporter.log("INFO-11089 is to verify Renaming the document type of workitem while passing supported character in the Workitem Name field test failed",	true);
			ATUReports.add("INFO-11089 is to verify Renaming the document type of workitem while passing supported character in the Workitem Name field test failed", LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Log.info("INFO-11089 is to verify Renaming the document type of workitem while passing supported character in the Workitem Name fieldfield");
			Assert.fail("INFO-11089 is to verify Renaming the document type of workitem while passing supported character in the Workitem Name field");
		} finally {
			Log.endTestCase("INFO_11089_Test to verify Renaming the document type of workitem while passing supported character in the Workitem Name field");
		}
	}
}
