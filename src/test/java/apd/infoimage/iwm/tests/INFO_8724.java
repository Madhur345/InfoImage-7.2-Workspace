...
/**
 * @author PradhanJ INFO-8724 Performing 'Document splits' operation
 *         using multiple images with different formats 
 *         and different class for a document type of workitem which is retrieved from the workstep
 */
public class INFO_8724 extends SuperClassIWM {
	@BeforeMethod
	public void setUp() {
	...
	public void testDocSplitUsingDifferentClassAfterRetrieving() {
		Log.startTestCase("INFO_8724_DocSplitUsingDifferentClassAfterRetrieving");

		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-8724  is To verify 'Document split' operation using multpile images "
				+ "with different formats and different class for "
				+ "a document type of workitem which is retrieved from the workstep");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				selectClass.selectByValue("invoice");
				...
				if (contentListInSecondSplitted == 3) {
					...
					ATUReports.add("Verify document Split by adding form fields and multiple pages "
							+ "using different class after retrieving from workstep test is successfull",
							true);
			...
			Reporter.log(
					"Verify document Split for a document type of "
					+ "workitem using different class which is retrieved from the workstep test is failed",
					true);
			ATUReports.add(
					"Verify document Split for a document"
					+ " type of workitem using different class which is retrieved from the workstep test is failed",
					true);
			Assert.fail(
					"Verify document Split for a document "
					+ "type of workitem using different class which is retrieved from the workstep test is failed");
		} finally {
			Log.endTestCase("INFO_8724_DocSplitUsingDifferentClassAfterRetrieving");
		}
	}
