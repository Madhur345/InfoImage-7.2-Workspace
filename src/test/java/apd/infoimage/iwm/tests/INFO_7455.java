 ...
 * @author SumanGaK
 * INFO-7455
 * This class sends workitem to different User Queue and checks the same workitem in new user Queue
 */
public class INFO_7455 extends SuperClassIWM
{
	@BeforeMethod
	public void setUp(){
	...
	public void tearDown(){
		...
		Log.endTestCase("INFO_7455_SendWorkitemToDifferentUserQueueAndCheckingSameWorkitemInThatUserQueueTest");
	}

	@SuppressWarnings("deprecation")
	@Test(enabled =true,priority=1,groups={"Inbox"})
	public void testSendWorkitemToDifferentUserQueueAndCheckingSameWorkitemInThatUserQueue()
	{
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-7455 is for sending workitem to different User Queue and checks the same workitem in new user Queue");
		...
		try
		{
			...
			Log.startTestCase("INFO_7455_SendWorkitemToDifferentUserQueueAndCheckingSameWorkitemInThatUserQueueTest");
			...
			Reporter.log("failed to execute SendWorkitemToDifferentUserQueueAndCheckingSameWorkitemInThatUserQueue test",true);
			ATUReports.add("failed to execute SendWorkitemToDifferentUserQueueAndCheckingSameWorkitemInThatUserQueue test",LogAs.FAILED,new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("failed to execute SendWorkitemToDifferentUserQueueAndCheckingSameWorkitemInThatUserQueue test");
		}
