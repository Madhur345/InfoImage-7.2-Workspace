...
/**
 * @author Biswajit - 07-June-2018 INFO_7575 This class Perform To Export 
 * JPG Image To BMP Format From Document Type Of Workitem In IWM
 */
public class INFO_7575 extends SuperClassIWM {
	...
	public void ExportJPGImageToBMPFormat() {

		String workitem = "wi" + util.getSysDate(0, "yyDDMMhhmmss");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase(
				"INFO-7575 This Scenario will export JPG image to BMP format from document type of workitem in IWM");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				Reporter.log("BMP format document successfully downloaded", true);
				ATUReports.add("BMP format document successfully downloaded", true);
				Thread.sleep(20000);
			...
			e.printStackTrace();
			Reporter.log("Exporting JPG image to BMP format from document type of workitem is failed.", true);
			ATUReports.add("Exporting JPG image to BMP format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting JPG image to BMP format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO-7575 export JPG image to BMP format from document type of workitem in IWM is Failed");
		}
	}
