 ...
 * @author SumanGaK
 * INFO-4312
 * This class is used to verify the Nested search functionality with Multi OR using query builder in iwm. 
 */
public class INFO_4312 extends SuperClassIWM{
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiOR() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-4312 is Test to verify the Nested search functionality with Multi OR using query builder in iwm.");
		ATUReports.setAuthorInfo("Suman", "JUN-2018", "0.3");

		try {
			Log.startTestCase("INFO_4312_VerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiORTest");
			...
			String nestedQuery=ExcelLib.getCellValue(xlpath, sheet, 68, 1);
			...
			if(workitemPresence){
				Reporter.log("Workitem is searched successfully by Nested query using multi OR",true);
				ATUReports.add("Workitem is searched successfully by Nested query using multi OR", true);
				Log.info("Workitem is searched successfully by Nested query using multi OR");
			}else{
				ATUReports.add("Nested query using multi OR test failed",LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query using multi OR test failed ");
			}
		}
		catch (Exception e) {
			e.printStackTrace();
			Log.error(e.getMessage());
			ATUReports.add("Failed to verify the Nested search functionality with multi OR using query builder in iwm.",LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality with multi OR using query builder test failed");
		}
		finally {
			Log.endTestCase("INFO_4312_VerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiORTest");
