 ...
 * @author PradhanJ
 * INFO-11806
 * Test to verify displaying of recent search actions using index field 
 * by adding the url extension for document and folder type of workitem
 */
public class INFO_11806 extends SuperClassIWM{
	...
	@Test
	public void testRecentSearchByAddingUrlExtensionForDocTypeWitem() {
		Log.startTestCase("INFO_11806_RecentSearchByAddingUrlExtensionForDocTypeWitem");
		try {
			ATUReports.setTestCaseReqCoverage("This Scenario INFO_11806  is Test to verify displaying of recent search "
					+ "actions using index field  by adding the url extension for document type of workitem'");
			...
			Reporter.log("Url by using index field for document type workitem is fetched from test data", true);
			ATUReports.add("Url by using index field for document type workitem  is fetched from test data", true);
			Log.info("Url by using index field for document type workitem  is fetched from test data");

			ATUReports.add("Url by using index field for document type workitem  is \n" + urlWdIndex, true);
			Reporter.log("Url by using index field for document type workitem  is \n" + urlWdIndex, true);
			...
			if (resultPresent) {
				...
				Reporter.log("Recent search Validation successful for url by using index field for document type workitem",
						true);
				ATUReports.add("Recent search Validation successful for url by using index field for document type workitem ",
						true);

				Log.info("Recent search Validation successful for url by using index field for document type workitem");
			} else {
				
				Assert.fail("Recent search Validation FAILED for url by using index field for document type workitem");
			}
		} catch (Exception e) {
			Reporter.log("Recent search Validation FAILED for url by using index field for document type workitem", true);
			ATUReports.add("Recent search Validation FAILED for url by using index field for document type workitem",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Recent search Validation FAILED for url by using index field for document type workitem");
		} finally {
			Log.endTestCase("INFO_11806_RecentSearchByAddingUrlExtensionForDocTypeWitem");
		}
	}
}
