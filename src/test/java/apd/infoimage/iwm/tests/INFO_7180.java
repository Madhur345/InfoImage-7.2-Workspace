...
/**
 * @author PradhanJ INFO-7180 To export PNG image as PNG format from document
 *         type of workitem in IWM
 */
public class INFO_7180 extends SuperClassIWM {
	...
	public void testExportPNGimageAsPNGimageFromDocTypeWorkitem() {

		Log.startTestCase("INFO_7180_ExportPNGimageAsPNGimageFromDocTypeWorkitem");

		ATUReports.setTestCaseReqCoverage(
				"INFO-7180 This Scenario is To export PNG image as PNG format from document type of workitem in IWM");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				Log.info("PNG format document successfully downloaded");
				util.waitForPageToLoad();
				util.wait(time);
			}
			...
			e.printStackTrace();
			Reporter.log("Exporting PNG image to PNG format from document type of workitem is failed.", true);
			ATUReports.add("Exporting PNG image to PNG format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting PNG image to PNG format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO_7180_ExportPNGimageAsPNGimageFromDocTypeWorkitem");
