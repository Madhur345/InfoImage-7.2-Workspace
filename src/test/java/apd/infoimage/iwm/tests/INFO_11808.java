 ...
 * @author PradhanJ
 * INFO-11808
 * Test to verify displaying of folder type of workitems using recent search actions	
 */
public class INFO_11808 extends SuperClassIWM{
	@BeforeMethod
	...
	@Test
	public void testVerifyDisplayingFolderTypeWitemsUsingRecentSearch() {
		Log.startTestCase("INFO_11808_VerifyDisplayingFolderTypeWitemsUsingRecentSearch");
		try {
			ATUReports.setTestCaseReqCoverage("This Scenario INFO_11808  is Test to verify displaying "
					+ "of folder type of workitems using recent search actions	");
			...
			if ( folderTypeWitemPresent) {

				Reporter.log("Recent search Validation successful for  folder type workitem",
						true);
				ATUReports.add("Recent search Validation successful for  folder type workitem ",
						true);

				Log.info("Recent search Validation successful for  folder type workitem");
			} else {
				
				//Assert.fail("Recent search Validation FAILED for folder type workitem");
			}
		} catch (Exception e) {
			Reporter.log("Recent search Validation FAILED for folder type workitem", true);
			//ATUReports.add("Recent search Validation FAILED for folder type workitem",
					//LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			//Assert.fail("Recent search Validation FAILED for  folder type workitem");
		} finally {
			Log.endTestCase("INFO_11808_VerifyDisplayingFolderTypeWitemsUsingRecentSearch");
		}
	}
}
