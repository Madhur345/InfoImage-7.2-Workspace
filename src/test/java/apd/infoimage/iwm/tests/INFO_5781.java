 ...
 * "ATTRIBUTES" and "QUEUES" 
 * column names by joining two tables in SQL query string that is passed as an input
 */
public class INFO_5781 extends SuperClassIWM{
	...
	public void testNestedSearchBypassingATTRIBUTESandQUEUESInQueyString() {
		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-5781 is Test to verify the Nested search functionality "
				+ "in Domain level by passing ATTRIBUTES and QUEUES column names by joining "
 + "two tables in SQL query string that is passed as an input");
		ATUReports.setAuthorInfo("Jayashri", "JUNE-2018", "0.3");

		try {
			Log.startTestCase(
					"INFO_5781_NestedSearchBypassingATTRIBUTESandQUEUESInQueyString");
			...
			if(f9){
				Reporter.log("Workitem is searched successfully by passing ATTRIBUTES and QUEUES "
						+ "column names by joining two tables in query string",true);
				ATUReports.add("Workitem is searched successfully by passing ATTRIBUTES and QUEUES column names "
						+ "by joining two tables in query string", true);
				Log.info("Workitem is searched successfully by passing ATTRIBUTES and QUEUES column names "
						+ "by joining two tables in query string");
			}else{
				ATUReports.add("Nested query using by passing ATTRIBUTES and QUEUES column names by "
						+ "joining two tables in query string test failed", 
						LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query by passing ATTRIBUTES and QUEUES column names test failed ");
			}
			...
			ATUReports.add(
					...
					+ "by passing ATTRIBUTES and QUEUES column names using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality by passing ATTRIBUTES and QUEUES column names"
					+ " using query builder in iwm test failed");
		}
		finally {
			Log.endTestCase("INFO_5781_NestedSearchBypassingATTRIBUTESandQUEUESInQueyString");
