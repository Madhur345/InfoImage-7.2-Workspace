...
/**
 * @author SinghAvn INFO-11042 Test to verify icon indicating that the workitems
 *         is in workflow for folder type of workitem which is retrieved from
 *         default queue
 */
public class INFO_11042 extends SuperClassIWM {
	@BeforeMethod
	public void setUp() {
	...
	public void testVeifySavingCustomJSForm() {
		Log.startTestCase("INFO_11042_To_Verify_Icon_Indicating_Workitems_Workflow_For_Folder_Type");
		ATUReports.setTestCaseReqCoverage("This Scenario will Test to verify icon indicating that the workitems "
				+ "is in workflow for folder type of workitem which is retrieved from default queue");
		ATUReports.setAuthorInfo("SinghAvn", "27-JUL-2018", "0.3");
		try {
			prop = new Properties();
			...
			ATUReports.add("Verify Icon for retrieved workitem of dovument type test failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Log.info("Verify Icon for retrieved workitem of folder type test failed");
			Assert.fail("Verify Icon for retrieved workitem of folder type test failed");
		} finally {
			Log.endTestCase("INFO_11042_VerifyIconForRetrievedWorkitemOfDocumentType");
		}
	}
}
