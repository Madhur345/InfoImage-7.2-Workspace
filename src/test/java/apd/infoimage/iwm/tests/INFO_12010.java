 ...
 * @author PradhanJ
 * INFO-12010
 * To verify search criteria for multiple folder type of workitems 
 * present in the queue using url extension in ENTERPRISE level	
 */
public class INFO_12010 extends SuperClassIWM{
	@BeforeMethod
	...
	public void testVerifySearchMultipleWitemsFolderTypeEnterpriseLevelUsingurlExtension()
	{
		Log.startTestCase("INFO_12010_VerifySearchMultipleWitemsFolderTypeEnterpriseLevelUsingurlExtension");
		try
		{
			ATUReports.setTestCaseReqCoverage("This Scenario INFO-12010  To verify search criteria for multiple "
					+ "folder type of workitems  present in the queue using url extension in ENTERPRISE level	 ");
			...
			String urlWdIndex = ExcelLib.getCellValue(xlpath, sheet2, 31, 1);
			...
			{*/
				Reporter.log("Search for Multiple folder type workitems validation successful"
						+ "using Url extenxion in Enetrprise Level", true);
				ATUReports.add("Search for Multiple folder type workitems validation successful"
						+ "using Url extenxion in Enetrprise Level", true);
				Log.info("Search for Multiple folder type workitems validation successful"
						+ "using Url extenxion in Enetrprise Level");
			/*}
			else
			{
				Assert.fail("Search for Multiple folder type workitems validation failed"
						+ "using Url extenxion in Enetrprise Level");
			}*/
		}
		catch (Exception e) {
			Reporter.log("Search for Multiple folder type workitems validation failed"
						+ "using Url extenxion in Enetrprise Level", true);
			ATUReports.add("Search for Multiple folder type workitems validation failed"
						+ "using Url extenxion in Enetrprise Level",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Search for Multiple folder type workitems validation failed"
						+ "using Url extenxion in Enetrprise Level");
		} finally {
			Log.endTestCase("INFO_12010_VerifySearchMultipleWitemsFolderTypeEnterpriseLevelUsingurlExtension");
		}
	}
}
