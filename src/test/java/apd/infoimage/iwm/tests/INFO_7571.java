...
/**
 * @author Biswajit - 11-June-2018 INFO_7571 This class Perform To Export 
 * JPG Image To PNG Format From Document Type Of Workitem In IWM
 */
public class INFO_7571 extends SuperClassIWM {
	...
	public void ExportJPGImageToPNGFormat() {

		String workitem = "wi" + util.getSysDate(0, "yyDDMMhhmmss");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase(
				"INFO-7571 This Scenario will export JPG image to PNG format from document type of workitem in IWM");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				Reporter.log("Export to PNG button clicked", true);
				ATUReports.add("Export to PNG button clicked", true);
				Thread.sleep(2000);
			...
			e.printStackTrace();
			Reporter.log("Exporting JPG image to PNG format from document type of workitem is failed.", true);
			ATUReports.add("Exporting JPG image to PNG format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting JPG image to PNG format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO-7571 export PNG image to JPG format from document type of workitem in IWM is Failed");
		}
	}
