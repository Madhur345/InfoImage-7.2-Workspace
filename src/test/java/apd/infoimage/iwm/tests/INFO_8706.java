 ...
 * INFO_8706 
 * This class Perform Document split operation using multiple images with Pdf format and same class for a
 * document type of workitem
 */
public class INFO_8706
extends SuperClassIWM {
	...
	public void testPerformingDocumentSplitMultipleImagesWithPdfFormatSameClassForDocumentWorkitem() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-8706 is for Performing Document split operation using multiple images with Pdf format and same class for a document type of workitem");
		ATUReports.setAuthorInfo("Suman", "MAY-2018", "0.3");

		try {
			DOMConfigurator.configure("log4j.xml");
			Log.startTestCase("INFO_8706_PerformingDocumentSplitMultipleImagesWithDifferentPdfSameClassForDocumentWorkitemTest");
			prop = new Properties();
			...
			String fileName = ExcelLib.getCellValue(xlpath, sheet, 62, 1);
			String secondFileName = ExcelLib.getCellValue(xlpath, sheet, 63, 1);
			String thirdFileName = ExcelLib.getCellValue(xlpath, sheet, 65, 1);
			...
			int numberThree = ExcelLib.getCellValueInt(xlpath,sheet,85,1);			
			...
			if (addNewPageWinPresent) {
				...
				String imagePath = System.getProperty("user.dir") + ExcelLib.getCellValue(xlpath, sheet, 28, 1);
				...
				Reporter.log("SampleDoc pdf file is uploaded", true);
				ATUReports.add("SampleDoc pdf file is uploaded", true);
				Log.info("SampleDoc pdf file is uploaded");
				...
				if (addSecondNewPageWinPresent) {
					...
					String secondimagePath = System.getProperty("user.dir")	+ ExcelLib.getCellValue(xlpath, sheet, 61, 1);
					...
					Reporter.log("show pdf file is uploaded", true);
					ATUReports.add("show pdf file is uploaded", true);
					Log.info("show pdf file is uploaded");
					...
					if (addThirdNewPageWinPresent) {
						...
						String thirdImagePath = System.getProperty("user.dir") + ExcelLib.getCellValue(xlpath, sheet, 64, 1);
				...
				if (contentListInSecondSplitted == numberThree) {
			...
			Reporter.log("Verify document Split by adding multiple Images With pdf format and same class name test is failed",true);
			ATUReports.add("Verify document Split by adding multiple Images With pdf format and same class name test is failed",true);
			Assert.fail("Verify document Split by adding multiple Images With pdf format and same class name test is failed");
		} 
		finally {
			Log.endTestCase("INFO_8706_PerformingDocumentSplitMultipleImagesWithDifferentPdfSameClassForDocumentWorkitem");
		}		
	}	
}