 ...
 * @author SumanGaK 
 * INFO-4315 
 * This class is used to verify the Nested search functionality with Multi ORs AND using query builder in iwm.
 */
public class INFO_4315 extends SuperClassIWM {
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiORsAND() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-4315 is Test to verify the Nested search functionality with Multi ORs AND using query builder in iwm.");
		ATUReports.setAuthorInfo("Suman", "JUN-2018", "0.3");

		try {
			Log.startTestCase("INFO_4315_VerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiORsANDTest");
			...
			String nestedQuery = ExcelLib.getCellValue(xlpath, sheet, 70, 1);
			...
			Reporter.log("Workitem is searched successfully by Nested query using multiORsAND", true);
			ATUReports.add("Workitem is searched successfully by Nested query using multiORsAND", true);
			Log.info("Workitem is searched successfully by Nested query using multiORsAND");

		} catch (Exception e) {
			e.printStackTrace();
			Log.error(e.getMessage());
			ATUReports.add("Failed to verify the Nested search functionality with multiORsAND using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));

		} finally {
			Log.endTestCase("INFO_4315_VerifyNestedSearchFunctionalityUsingQueryBuilderWithMultiORsANDTest");
