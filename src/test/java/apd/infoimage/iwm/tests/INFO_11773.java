 ...
 * @author PradhanJ
 * INFO-11773
 * To verify search criteria for multiple document type of workitems using url extension in DOMAIN level
 */
public class INFO_11773 extends SuperClassIWM{
	@BeforeMethod
	...
	public void testSearchMultipleDocTypeDomainLevelUsingUrlExtension() {
		Log.startTestCase("INFO_11773_SearchMultipleDocTypeDomainLevelUsingUrlExtension");
		try {
			ATUReports.setTestCaseReqCoverage("This Scenario INFO-11773   To verify search criteria for multiple "
					+ "document type of workitems using url extension in DOMAIN level ");
			...
			Reporter.log("Search for multiple document type workitems validation failed"
					+ "using Url extenxion in Domain Level", true);
			ATUReports.add(
					"Search for multiple document type workitems validation failed"
							+ "using Url extenxion in Domain Level",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Search for multiple document type workitems validation failed"
					+ "using Url extenxion in Domain Level");
		} finally {
			Log.endTestCase("INFO_11773_SearchMultipleDocTypeDomainLevelUsingUrlExtension");
		}
	}
}
