 ...
 * @author PradhanJ
 * INFO-7181
 * To export PNG image as JPEG format from document type of workitem in IWM
 */
public class INFO_7181 extends SuperClassIWM {
	...
	public void testExportPNGImageAsJPEGFromDocTypeWorkitem() {

		Log.startTestCase("INFO_7181_ExportPNGImageAsJPEGFromDocTypeWorkitem");

		ATUReports.setTestCaseReqCoverage(
				"INFO-7181 This Scenario is To export PNG image as JPEG format from document type of workitem in IWM");
		
		ATUReports.setAuthorInfo("Jayashri", "June-2018", "1.0");
		try {
			prop = new Properties();
			...
			if (addNewPageWinPresent) {
				...
				Log.info("checkbox button clicked");
				util.wait(time);

				wdp.getExportImage().click();
				Reporter.log("Export Image button clicked", true);
				ATUReports.add("Export Image button clicked", true);
				Log.info("Export Image button clicked");
				util.wait(time);

				util.jclick(wdp.getuploadJPEG());
				Reporter.log("JPEG button clicked", true);
				ATUReports.add("JPEG button clicked", true);
				Log.info("JPEG button clicked");
				util.wait(time);

				util.jclick(wdp.getDownloadButton());
				Reporter.log("JPEG format document successfully downloaded", true);
				ATUReports.add("JPEG format document successfully downloaded", true);
				Log.info("JPEG format document successfully downloaded");
				util.waitForPageToLoad();
				util.wait(time);
			}
			...
			e.printStackTrace();
			Reporter.log("Exporting PNG image to JPEG format from document type of workitem is failed.", true);
			ATUReports.add("Exporting PNG image to JPEG format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting PNG image to JPEG format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO_7181_ExportPNGImageAsJPEGFromDocTypeWorkitem");
