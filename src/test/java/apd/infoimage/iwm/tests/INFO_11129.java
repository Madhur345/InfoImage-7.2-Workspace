...
/**
 * @author DashBisw INFO_11129 This class is To Performing Document Duplicate Operation 
 * With Same Different Class For Document Type Of Workitem Which is  Filed Inside
 * Folder With Index Fields Defined". 27/08/2018
 */
import java.io.File;
...
public class INFO_11129 extends SuperClassIWM {
	...
	public void testPerformingDocumentDuplicateOperationWithSameDifferentClassForDocumentTypeOfWorkitem() {
		...
		ATUReports.setTestCaseReqCoverage(
				"INFO_11129_PerformingDocumentDuplicateOperationWithSameDifferentClassForDocumentTypeOfWorkitemWhichisFiledInsideFolderWithIndexFieldsDefined");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				hp.searchByNameInWorkitemTabAndDisplay(newSpliwtItem);
				Reporter.log("New workitem will be created with those pages", true);
				ATUReports.add("New workitem will be created with those pages", true);
				Log.info("New workitem will be created with those pages");

			}

		} catch (Exception e) {
			e.printStackTrace();
			Reporter.log("INFO_11129 Failed", true);
			Log.info("INFO_11129 Failed");
			ATUReports.add("INFO_11129 Failed", LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
