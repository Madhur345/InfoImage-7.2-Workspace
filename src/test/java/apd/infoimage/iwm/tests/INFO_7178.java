 ...
 * @author PradhanJ
 * INFO-7178
 * To export PNG image as TIF format from document type of workitem in IWM
 */
public class INFO_7178 extends SuperClassIWM {
	...
	public void testExportPNGImageAsTIFFormatFromDocTypeWorkitem() {

		Log.startTestCase("INFO_7178_ExportPNGImageAsTIFFormatFromDocTypeWorkitem");

		ATUReports.setTestCaseReqCoverage(
				"INFO-7189 This Scenario is To export PNG image as TIF format from document type of workitem in IWM");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				Reporter.log("TIF button clicked", true);
				ATUReports.add("TIF button clicked", true);
				Log.info("TIF button clicked");
				util.wait(time);
				...
				Log.info("TIF format document successfully downloaded");
				util.waitForPageToLoad();
				util.wait(time);
			}
			...
			e.printStackTrace();
			Reporter.log("Exporting PNG image to TIF format from document type of workitem is failed.", true);
			ATUReports.add("Exporting PNG image to TIF format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting PNG image to TIF format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO_7178_ExportPNGImageAsTIFFormatFromDocTypeWorkitem");
