 ...
 * INFO-7263
 * This class verifies 150 * 150 DPI images processing through iwm.
 * Pre-Condition : Test date should be provided with 150 * 150 DPI image.
 */
public class INFO_7263 extends SuperClassIWM{
	...
	public void testVerifyImageDPI(){
		...
		try {
			Log.startTestCase("INFO_7263_VerifyImageDPITest");
			...
			String fileName = ExcelLib.getCellValue(xlpath,sheet,14,1);
			...
			if(addNewImportsWinPresent)
			{
				...
				String imagePath = System.getProperty("user.dir")+ExcelLib.getCellValue(xlpath,sheet,12,1);
				...
				{
					Reporter.log("Selected tiff file name validation is successful.", true);
					ATUReports.add("Selected tiff file name validation is successful.", true);
					Log.info("Selected tiff file name validation is successful.");
				}
				else
				{
					Reporter.log("Selected tiff file name validation is failed.", true);
					ATUReports.add("Selected tiff file name validation is failed.", true);
					Log.info("Selected tiff file name validation is failed.");
					Assert.fail("Selected tiff file name validation is failed.");					
				}
				util.wait(time);
				util.wait(time);
				...
				try
				{
					...
					{
						Reporter.log("TIFF File is imported successfully. ", true);
						ATUReports.add("TIFF File is imported successfully. ", true);
						Log.info("TIFF File is imported successfully. ");
					}
					else
					{
						Reporter.log("TIFF File is NOT imported successfully.", true);
						ATUReports.add("TIFF File is NOT imported successfully.", true);
						Log.info("TIFF File is NOT imported successfully. ");
						Assert.fail("TIFF File is NOT imported successfully.");
					}
				}
				catch(Exception e1)
				{
			...
			else
			{
				...
				ATUReports.add("TIFF File is downloaded successfully.", true);
				Log.info("TIFF File is downloaded successfully. ");
				ATUReports.add("Verify 150 * 150 DPI image processed through iwm", "","150 * 150 DPI of image should be displayed",
