 ...
 * @author SumanGaK
 * INFO-4317
 * This class is used to verify the Nested search functionality Using Query Builder With Invalid Data in iwm. 
 */
public class INFO_4317 extends SuperClassIWM{
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchFunctionalityUsingQueryBuilderWithInvalidData() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-4317 is Test to verify the Nested search functionality with Invalid Data using query builder in iwm.");
		ATUReports.setAuthorInfo("Suman", "JUN-2018", "0.3");

		try {
			Log.startTestCase("INFO_4317_VerifyNestedSearchFunctionalityUsingQueryBuilderWithInvalidDataTest");
			...
			String invalidData=ExcelLib.getCellValue(xlpath,sheet,72,1);
			...
			sp.getIdCodeFieldInNestedSearch().sendKeys(invalidData);
			Reporter.log("ID Code invalid Data value is entered", true);
			Log.info("ID Code invalid Data value is entered");
			ATUReports.add("ID Code invalid Data value is entered", true);
			...
			if(!messagePresence){
				...
				ATUReports.add("Nested Search Functionality Using Query Builder With Invalid Data test is failed",LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested Search Functionality Using Query Builder With Invalid Data test is failed");
			}			
		}
		catch (Exception e) {
			e.printStackTrace();
			Log.error(e.getMessage());
			ATUReports.add("Failed to verify the Nested Search Functionality Using Query Builder With Invalid Data in iwm.",LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested Search Functionality Using Query Builder With Invalid Data test is failed");
		}
		finally {
			Log.endTestCase("INFO_4317_VerifyNestedSearchFunctionalityUsingQueryBuilderWithInvalidDataTest");
