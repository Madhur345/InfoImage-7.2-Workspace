 ...
 * @author PradhanJ
 * INFO-5795
 * To test the nested Query in IWM in Domain level by passing 
 * AND/OR condition in SQL query string that is passed as an input.
 */
public class INFO_5795 extends SuperClassIWM{
	...
	public void testVerifyNestedSearchBypassingANDcondition() {
		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-5795 is Test to verify the Nested search functionality "
				+ "with AND/OR condition in SQL query string that is passed as an input");
		ATUReports.setAuthorInfo("Jayashri", "JUNE-2018", "0.3");

		try {
			Log.startTestCase(
					"INFO_5795_VerifyNestedSearchBypassingANDcondition");
			...
			if(f9){
				Reporter.log("Workitem is searched successfully by Nested query using AND operator",true);
				ATUReports.add("Workitem is searched successfully by Nested query using AND operator", true);
				Log.info("Workitem is searched successfully by Nested query using AND operator");
			}else{
				ATUReports.add("Nested query using AND operator test failed", 
						LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query using AND operator test failed ");
			}
			...
			ATUReports.add(
					...
					+ "with AND using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality with AND test failed");
		}
		finally {
			Log.endTestCase("INFO_5795_VerifyNestedSearchBypassingANDcondition");
