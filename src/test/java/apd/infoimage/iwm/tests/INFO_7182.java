 ...
 * @author PradhanJ
 * INFO-7182
 * To export PNG image as JPG format from document type of workitem in IWM
 */
public class INFO_7182 extends SuperClassIWM {
	...
	public void testExportPNGImageAsJPGFormatFromDocTypeWorkitem() {

		Log.startTestCase("INFO_7182_ExportPNGImageAsJPGFormatFromDocTypeWorkitem");

		ATUReports.setTestCaseReqCoverage(
				"INFO-7182 This Scenario is To export PNG image as JPG format from document type of workitem in IWM");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				util.jclick(wdp.getuploadJPG());
				Reporter.log("JPG button clicked", true);
				ATUReports.add("JPG button clicked", true);
				Log.info("JPG button clicked");
				util.wait(time);

				util.jclick(wdp.getDownloadButton());
				Reporter.log("JPG format document successfully downloaded", true);
				ATUReports.add("JPG format document successfully downloaded", true);
				Log.info("JPG format document successfully downloaded");
				util.waitForPageToLoad();
				util.wait(time);
			}
			...
			e.printStackTrace();
			Reporter.log("Exporting PNG image to JPG format from document type of workitem is failed.", true);
			ATUReports.add("Exporting PNG image to JPG format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting PNG image to JPG format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO_7182_ExportPNGImageAsJPGFormatFromDocTypeWorkitem");
