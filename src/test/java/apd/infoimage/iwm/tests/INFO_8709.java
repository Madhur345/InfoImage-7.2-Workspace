...
/**
 * @author Biswajit - 01-June-2018 INFO_8710 This class Perform Document split
 *         operation using multpile images with TIF format and Different class
 *         for a document type of workitem
 */
public class INFO_8709 extends SuperClassIWM {
	...
	public void testDocSplitDiffClass() {
		Log.startTestCase(
				"INFO_8709_PerformingDocumentSplitOperationUsingMultipleImagesWithTIFFormatAndDifferentClassForADocumentTypeOfWorkitem");

		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-8709  is To verify 'Document split' operation Using Multiple Images With TIF Format"
		...
		int time = ExcelLib.getCellValueInt(xlpath, sheet2, 58, 1);
		try {
			...
			if (addNewPageWinPresent) {
				...
				if (addNewPageWinPresent1) {
					...
					String imagePath2 = PNGFilePath + "\\singlePage.tif";
					...
					if (contentListSplitted == 2) {
						...
						ATUReports.add(
								"Verify document Split by adding form fields and multiple pages test is successful",
								true);
			...
			ATUReports.add(
					"verify 'Document split' operation Using Multiple Images With TIF Format with different class for a document type of workitem test is failed",
