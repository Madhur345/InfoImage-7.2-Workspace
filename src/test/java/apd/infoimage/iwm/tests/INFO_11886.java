 ...
 * @author PradhanJ
 * INFO-11886
 * Test to verify the search criteria using multiple index fields of any form 
 * by adding the url extension for document type of workitem
 */
public class INFO_11886 extends SuperClassIWM{
	@BeforeMethod
	...
	@Test
	public void testVerifyRecentSearchUsingMultipleIndexFieldsInUrlExtension() {
		Log.startTestCase("INFO_11886_VerifyRecentSearchUsingMultipleIndexFieldsInUrlExtension");
		try {
			ATUReports.setTestCaseReqCoverage("This Scenario INFO_11886  is Test to verify the search "
					+ "criteria using multiple index fields of any form "
					+ " by adding the url extension for document type of workitem");
			...
			cwp.getInvoiceNoInFormFields().clear();
			...
			String urlWdMultipleIndex = ExcelLib.getCellValue(xlpath, sheet2, 26, 1);
			Reporter.log("Url by using multiple index fields for document type workitem is fetched from test data", true);
			ATUReports.add("Url by using multiple index fields for document type workitem  is fetched from test data", true);
			Log.info("Url by using index multiple index fields for document type workitem  is fetched from test data");

			ATUReports.add("Url by using multiple index fields for document type workitem  is \n" + urlWdMultipleIndex, true);
			Reporter.log("Url by using multiple index fields for document type workitem  is \n" + urlWdMultipleIndex, true);
			...
			if (resultPresent) {
				...
				Reporter.log("Recent search Validation successful for url by using multiple index fields for document type workitem",
						true);
				ATUReports.add("Recent search Validation successful for url by using multiple index fields for document type workitem ",
						true);

				Log.info("Recent search Validation successful for url by using multiple index fields for document type workitem");
			} else {
				
				Assert.fail("Recent search Validation FAILED for url by using multiple index fields for document type workitem");
			}
		} catch (Exception e) {
			Reporter.log("Recent search Validation FAILED for url by using multiple index fields for document type workitem", true);
			ATUReports.add("Recent search Validation FAILED for url by using multiple index fields for document type workitem",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Recent search Validation FAILED for url by using multiple index fieldsfor document type workitem");
		} finally {
			Log.endTestCase("INFO_11886_VerifyRecentSearchUsingMultipleIndexFieldsInUrlExtension");
		}
	}
}
