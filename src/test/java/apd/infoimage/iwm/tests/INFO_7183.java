 ...
 * @author PradhanJ
 * INFO-7183
 * To export PNG image as GIF format from document type of workitem in IWM
 */
public class INFO_7183 extends SuperClassIWM {
	...
	public void testExportPNGImageAsGIFFormatFromDocTypeWorkitem() {

		Log.startTestCase("INFO_7183_ExportPNGImageAsGIFFormatFromDocTypeWorkitem");

		ATUReports.setTestCaseReqCoverage(
				"INFO-7183 This Scenario is To export PNG image as GIF format from document type of workitem in IWM");
		...
		try {
			...
			if (addNewPageWinPresent) {
				...
				Reporter.log("GIF button clicked", true);
				ATUReports.add("GIF button clicked", true);
				Log.info("GIF button clicked");
				util.wait(time);
				...
				Log.info("GIF format document successfully downloaded");
				util.waitForPageToLoad();
				util.wait(time);
			}
			...
			e.printStackTrace();
			Reporter.log("Exporting PNG image to GIF format from document type of workitem is failed.", true);
			ATUReports.add("Exporting PNG image to GIF format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting PNG image to GIF format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO_7183_ExportPNGImageAsGIFFormatFromDocTypeWorkitem");
