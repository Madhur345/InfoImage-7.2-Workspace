...
/**
 * @author Biswajit - 06-June-2018 INFO_7555 This class Perform To Export 
 * TIF Image To PDF Format From Document Type Of Workitem In IWM
 */
public class INFO_7555 extends SuperClassIWM {
	...
	public void testSameWorkIteminUserQueue() {
		...
		Log.startTestCase(
				"INFO-7555 This Scenario will export TIF image to PDF format from document type of workitem in IWM");
		...
		try {
			...
			e.printStackTrace();
			Reporter.log("Exporting TIF image to PDF format from document type of workitem is failed.", true);
			ATUReports.add("Exporting TIF image to PDF format from document type of workitem is failed", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Exporting TIF image to PDF format from document type of workitem is failed.");
			Log.error(e.getMessage());
		} finally {
			Log.endTestCase("INFO-7555 export TIF image to PDF format from document type of workitem in IWM is Failed");
		}
	}
