...
/**
 * @author SumanGaK - 29-May-2018 
 * INFO_8705 
 * This class Perform Document split operation using multiple images with tif format and same class for a
 * document type of workitem
 */
public class INFO_8705
extends SuperClassIWM {
	...
	public void testPerformingDocumentSplitMultipleImagesWithTifFormatSameClassForDocumentWorkitem() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-8705 is for Performing Document split operation using multiple images with tif format and same class for a document type of workitem");
		ATUReports.setAuthorInfo("Suman", "MAY-2018", "0.3");

		try {
			DOMConfigurator.configure("log4j.xml");
			Log.startTestCase("INFO_8705_PerformingDocumentSplitMultipleImagesWithTifFormatSameClassForDocumentWorkitemTest");
			...
			String thirdFileName = ExcelLib.getCellValue(xlpath, sheet, 26, 1);
			String newWitemStr = ExcelLib.getCellValue(xlpath, sheet, 20, 1);
			...
			if (addNewPageWinPresent) {
				...
				if (addSecondNewPageWinPresent) {
					...
					Log.info("FakePage2 TIf file is uploaded");
					...
					if (addThirdNewPageWinPresent) {
						...
						String thirdImagePath = System.getProperty("user.dir") + ExcelLib.getCellValue(xlpath, sheet, 25, 1);
						...
						Log.info("singlePage TIf file is uploaded");
				...
				if (contentListSplitted == numberOne) {
					...
					ATUReports.add("Verify document Split by adding multiple pages and same class test is successful",
							true);
				...
				if (contentListInSecondSplitted == numberTwo) {
					...
					ATUReports.add("Verify document Split by adding form fields and multiple pages test is successful",
							true);
			...
			Reporter.log("Verify document Split by adding multiple Images With Tif format and same class name test is failed", true);
			ATUReports.add("Verify document Split by adding multiple Images With Tif format and same class name test is failed", true);
			Assert.fail("Verify document Split by adding multiple Images With Tif format and same class name test is failed");
		} finally {
			Log.endTestCase("INFO_8705_PerformingDocumentSplitMultipleImagesWithTifFormatSameClassForDocumentWorkitem");
		}
	}
}
