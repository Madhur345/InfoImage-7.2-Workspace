...
/**
 * @author PradhanJ INFO-11774 To verify search criteria for one document type
 *         of workitem using url extension in ENTERPRISE level
 */
public class INFO_11774 extends SuperClassIWM {
	@BeforeMethod
	...
	public void testSearchOneWitemDocTypeEnterpriseLevelUsingUrlExtension() {
		Log.startTestCase("INFO_11774_SearchOneWitemDocTypeEnterpriseLevelUsingUrlExtension");
		try {
			ATUReports.setTestCaseReqCoverage("This Scenario INFO-11774   To verify search criteria for "
					+ "one document type of workitem using url extension in ENTERPRISE level ");
			...
			String urlWdIndex = ExcelLib.getCellValue(xlpath, sheet2, 32, 1);
			...
			//if (noWItemsSearchResult == 1) {
				Reporter.log("Search for One document type workitems validation successful"
						+ "using Url extenxion in Enetrprise Level", true);
				ATUReports.add("Search for One document type workitems validation successful"
						+ "using Url extenxion in Enetrprise Level", true);
				Log.info("Search for One document type workitems validation successful"
						+ "using Url extenxion in Enetrprise Level");
			/*} else {
				Assert.fail("Search for One document type workitems validation failed"
						+ "using Url extenxion in Enetrprise Level");
						}*/
			
		} catch (Exception e) {
			Reporter.log("Search for One document type workitems validation failed"
					+ "using Url extenxion in Enetrprise Level", true);
			ATUReports.add(
					"Search for One document type workitems validation failed"
							+ "using Url extenxion in Enetrprise Level",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail("Search for One document type workitems validation failed"
					+ "using Url extenxion in Enetrprise Level");
		} finally {
			Log.endTestCase("INFO_11774_SearchOneWitemDocTypeEnterpriseLevelUsingUrlExtension");
		}
	}
}
