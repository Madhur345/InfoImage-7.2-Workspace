 ...
 * @author PradhanJ
 * INFO-4299
 * Test to verify the Nested search functionality with 
 * (IN)operator using query builder in iwm.
 */
public class INFO_4299 extends SuperClassIWM{
	@BeforeMethod
	public void setUp() {
	...
	public void testVerifyNestedSearchWith_IN_Operator() {
		ATUReports.setTestCaseReqCoverage(
				"This Scenario INFO-4299 is Test to verify the Nested search functionality "
				+ "with (IN)operator using query builder in iwm.");
		ATUReports.setAuthorInfo("Jayashri", "MAY-2018", "0.3");

		try {
			Log.startTestCase(
					"INFO_4299_VerifyNestedSearchWith_IN_Operator");
			...
			if(f9){
				Reporter.log("Workitem is searched successfully by Nested query using <> operator",true);
				ATUReports.add("Workitem is searched successfully by Nested query using <> operator", true);
				Log.info("Workitem is searched successfully by Nested query using <> operator");
			}else{
				ATUReports.add("Nested query using <> operator test failed", 
						LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Nested query using <> operator test failed ");
			}
			...
			ATUReports.add(
					...
					+ "with <> operator using query builder in iwm.",
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality with  <> operator test failed");
		}
		finally {
			Log.endTestCase("INFO_4299_VerifyNestedSearchWith_IN_Operator");
		}
	}
}