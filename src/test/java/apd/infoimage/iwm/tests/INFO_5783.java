 ...
 * @author PradhanJ
 * INFO-5783
 * To test the nested Query in IWM for workitems in Domain level 
 * by passing only "ATTRIBUTES" column names in SQL query string that is passed as an input.
 */
public class INFO_5783 extends SuperClassIWM{
	...
	public void testNestedQueryInDomainLevelByPassingATTRIBUTESasColumn() {
 ...
 "by passing only ATTRIBUTES column names in SQL query string that is passed as an input.");
		...
		try {
			Log.startTestCase(
					"INFO_5783_NestedQueryInDomainLevelByPassingATTRIBUTESasColumn");
			...
			if(f9){
				Reporter.log("Workitem is searched successfully In Domain level by by passing only ATTRIBUTES "
						+ "column names in SQL query string that is passed as an input.",true);
				ATUReports.add("Workitem is searched successfully In Domain level by by passing only ATTRIBUTES "
						+ "column names in SQL query string that is passed as an input.", true);
				Log.info("Workitem is searched successfully In Domain level by passing only ATTRIBUTES column names in "
						+ "SQL query string that is passed as an input.");
			}else{
				ATUReports.add("Nested query by passing only ATTRIBUTES column names in SQL query string In Domain level test failed", 
			...
			ATUReports.add(
					...
					LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
			Assert.fail(" Nested search functionality In Domain level by passing only "
					+ "ATTRIBUTES column names in SQL query string test failed");
		}
		finally {
			Log.endTestCase("INFO_5783_NestedQueryInDomainLevelByPassingATTRIBUTESasColumn");
