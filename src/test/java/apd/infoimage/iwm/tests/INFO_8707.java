 ...
 * INFO_8707 
 * This class Perform Document split operation using multpile images with jpg format and same class for a
 * document type of workitem
 */
public class INFO_8707
extends SuperClassIWM {
	...
	public void testPerformingDocumentSplitMultipleImagesWithJpgFormatSameClassForDocumentWorkitem() {
		ATUReports.setTestCaseReqCoverage("This Scenario INFO-8707 is for Performing Document split operation using multiple images with jpg format and same class for a document type of workitem");
		ATUReports.setAuthorInfo("Suman", "MAY-2018", "0.3");

		try {
			DOMConfigurator.configure("log4j.xml");
			Log.startTestCase("INFO_8707_PerformingDocumentSplitMultipleImagesWithJpgFormatSameClassForDocumentWorkitemTest");
			...
			if (addNewPageWinPresent) {
				...
				if (addSecondNewPageWinPresent) {
					...
					String secondimagePath = System.getProperty("user.dir")
							+ ExcelLib.getCellValue(xlpath, sheet, 29, 1);
					...
					if (addThirdNewPageWinPresent) {
						...
						String thirdImagePath = System.getProperty("user.dir")
								+ ExcelLib.getCellValue(xlpath, sheet, 54, 1);
			...
			Reporter.log("Verify document Split by adding multiple Images With Jpg format and same class name test is failed",
					true);
			ATUReports.add("Verify document Split by adding multiple Images With Jpg format and same class name test is failed",
					true);
			Assert.fail("Verify document Split by adding multiple Images With Jpg format and same class name test is failed");
		} finally {
			Log.endTestCase("INFO_8707_PerformingDocumentSplitMultipleImagesWithJpgFormatSameClassForDocumentWorkitem");
		}
	}	
