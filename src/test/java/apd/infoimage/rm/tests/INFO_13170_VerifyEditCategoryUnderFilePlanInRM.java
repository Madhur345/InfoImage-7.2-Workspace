 ...
 * @author PradhanJ
 * INFO-13170
 * To test Edit Category under File plan in RM application
 */
public class INFO_13170_VerifyEditCategoryUnderFilePlanInRM extends SuperClassIWM{
	...
	@Test
	public void testVerifyEditCategoryUnderFilePlanInRM()
	{
		try {
			Log.startTestCase("INFO_13170_VerifyEditCategoryUnderFilePlanInRM");
			ATUReports.setTestCaseReqCoverage(
					"This Scenario INFO-13170  is To test Edit Category under File plan in RM application");
			...
			rmfpp.getEditCategoryButton().click();
			Reporter.log("Edit category button is clicked", true);
			ATUReports.add("Edit category button is clicked", true);
			Log.info("Edit category button is clicked");			
			util.waitForElementPresent(rmfpp.getEditCategoryPopup());
			
			rmfpp.getCatNameInEditCategory().clear();
			rmfpp.getCatNameInEditCategory().sendKeys(newCategoryName);
			Reporter.log("new category name is entered in the edit category popup", true);
			ATUReports.add("new category name is entered in the edit category popup", true);
			Log.info("new category name is entered in the edit category popup");
			
			rmfpp.getUpdateBtnInEditCategory().click();
			Reporter.log("Update button edit category popup is clicked ", true);
			ATUReports.add("Update button edit category popup is clicked ", true);
			Log.info("Update button edit category popup is clicked ");
			util.waitForPageToLoad();
			
			boolean editCategorySucMsg=util.verifyObjectPresentReturnsBool(rmfpp.getEditCategorySucMsg());
			if(editCategorySucMsg)
			{
				Reporter.log("Category is updated successfully message is dispalyed", true);
				ATUReports.add("Category is updated successfully message is dispalyed", true);
				Log.info("Category is updated successfully message is dispalyed");
			}
			else
			{
				Reporter.log("Category is NOT updated successfully", true);
				ATUReports.add("Category is NOT updated successfully", LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Category is NOT updated successfully");
			}
			boolean newCategoryPresent=util.verifyObjectPresentReturnsBool(rmfpp.getCategoryNameRadioInCategoryView(newCategoryName));
			
			if(newCategoryPresent)
			{
				Reporter.log("Updated Category is listed under Category view", true);
				ATUReports.add("Updated Category is listed under Category view", true);
				Log.info("Updated Category is listed under Category view");
			}
			else
			{
				Reporter.log("Edit Category test failed", true);
				ATUReports.add("Edit Category test failed", LogAs.FAILED, new CaptureScreen(ScreenshotOf.DESKTOP));
				Assert.fail("Edit Category test failed");
			}
			
		}
		catch (Exception e) {
			e.printStackTrace();
			Log.error("Edit Category under Series in File Plan test failed " + e.getMessage());
			Reporter.log("Edit Category under Series in File Plan test failed", true);
			Assert.fail("Edit Category under Series in File Plan test failed");
		}
		finally {
			Log.endTestCase("INFO_13170_VerifyEditCategoryUnderFilePlanInRM");
		}
	}
}
