 ...
 * @author PradhanJ
 * INFO-13169
 * To verify delete Series in RM application
 */
public class INFO_13169_VerifyDeleteSeriesInRM extends SuperClassIWM{
	@BeforeMethod
	...
	@Test
	public void test_VerifyDeleteSeriesInRM()
	{
		try {
			Log.startTestCase("INFO_13169_VerifyDeleteSeriesInRM");
			ATUReports.setTestCaseReqCoverage(
					"This Scenario INFO-13169  is To verify delete Series in RM application");
			...
			rmfpp.getDeleteSeriesButton().click();
			util.waitForElementPresent(rmfpp.getDeleteSeriesSucMsg());
			
			boolean deleteSerSucMsgPresent=util.verifyObjectPresentReturnsBool(rmfpp.getDeleteSeriesSucMsg());
			if(deleteSerSucMsgPresent)
			{
				Reporter.log("Series is deleted successfully message is dispalyed", true);
				ATUReports.add("Series is deleted successfully message is dispalyed", true);
				Log.info("Series is deleted successfully message is dispalyed");
			}
			else
			{
			...
			e.printStackTrace();
			Log.error("Delete Series in File Plan test failed " + e.getMessage());
			Reporter.log("Delete Series in File Plan test failed", true);
			Assert.fail("Delete Series in File Plan test failed");
		}
		finally {
			Log.endTestCase("INFO_13169_VerifyDeleteSeriesInRM");
		}
	}
}
